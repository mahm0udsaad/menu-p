...
interface VintageBakeryTemplateProps {
...
export default function VintageBakeryTemplate({
  restaurant,
  categories,
  appliedFontSettings,
  appliedPageBackgroundSettings,
  appliedRowStyles,
  currentLanguage = 'ar'
}: VintageBakeryTemplateProps) {
  const isRTL = ['ar', 'fa', 'ur', 'he'].includes(currentLanguage);
  const currency = restaurant.currency || '$';

  return (
    <div className="vintage-bakery-template min-h-[1123px] p-8 relative bg-gradient-to-br from-amber-50 via-orange-50 to-brown-50">
      {/* Content */}
        ...
        <header className="text-center mb-16">
          <div className="mb-8">
            <h1 className="text-6xl font-black text-brown-800 mb-4 tracking-wider" style={{
              ...
              {restaurant.name?.toUpperCase() || "VINTAGE BAKERY"}
            </h1>
            ...
            <p className="text-2xl font-medium text-brown-600 tracking-wide" style={{
              ...
              {isRTL ? 'مخبز كلاسيكي' : 'CLASSIC VINTAGE BAKERY'}
            </p>
          </div>
        </header>
        ...
        <div className="mt-20 text-center">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 border-2 border-brown-200">
            ...
            <p className="text-sm text-brown-500 mt-4" style={{
              ...
              {isRTL ? 'حلويات طازجة يومياً' : 'FRESH BAKED GOODS DAILY'}
            </p>
          </div>
        </div>
      </div>
